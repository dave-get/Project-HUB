...
export function ProposalCard({proposals, layout}:{ proposals: Submission; layout: "grid" | "list" }) {
  const status = proposals?.status?.toLowerCase()
    ...
    return (
      <Card className="h-[280px] flex flex-col">
        <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
          <CardTitle className="text-lg font-medium line-clamp-2">{proposals?.title}</CardTitle>
          {status === "pending" && <Badge className="bg-amber-500">Pending</Badge>}
          {status === "approved" && <Badge className="bg-green-500">Approved</Badge>}
          {status === "rejected" && <Badge className="bg-red-500">Rejected</Badge>}
        </CardHeader>
        <CardContent className="flex-grow">
          <div className="space-y-2">
            <div className="text-sm">
              <span className="font-semibold">Department:</span> {proposals.student.department}
            </div>
            <div className="text-sm text-muted-foreground">{proposals?.student?.fullName}</div>
          </div>
        </CardContent>
